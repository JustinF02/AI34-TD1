...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Stochastic Optimization & Tree Exploration with @tree library
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Number of stochastic trials to perform
num_trials = 3;  

% Initialize variables to store the best cost and trajectory
bestCost = inf;  % A large initial value for the best cost
bestTrajectory = [];  % To store the best trajectory

% Generate several trees and apply stochastic optimization
for trial = 1:num_trials
    fprintf('Stochastic Optimization Trial %d:\n', trial);
    
    % Generate a fresh tree for each trial
    currentTree = ComputeGraphBasedCircularLimitCycles();  

    % Perform stochastic optimization by randomly discarding branches
    % Here, we randomly remove 1/10 of the branches to reduce the search space
    totalNodes = currentTree.nnodes;
    nodesToRemove = randperm(totalNodes, floor(2/3 * totalNodes)); 
    
    % Remove these randomly selected nodes
    for i = 1:length(nodesToRemove)
        currentTree = removeNode(currentTree, nodesToRemove(i));
    end
    disp(['Remaining nodes after removal: ', num2str(currentTree.nnodes)]);

    
    % Find the shortest path on the reduced tree
    optimizedTrajectory = TreeProcess(currentTree);  
    
    % Compute cost of the current trajectory (you can use a custom cost function)
    currentCost = ComputeCost(optimizedTrajectory);  % Replace this with your actual cost function
    
    % Display the optimized trajectory for the current trial
    plotOptimizedPath(optimizedTrajectory);
    title(sprintf('Optimized Path - Trial %d', trial));
    
    % Pause to view the result before the next trial
    pause(2);  
    
    % Check if this is the best trajectory so far
    if currentCost < bestCost
        bestCost = currentCost;  % Update the best cost
        bestTrajectory = optimizedTrajectory;  % Update the best trajectory
    end
end

% Now plot only the most optimized trajectory
figure;
hold on;
plot(bestTrajectory(:,1), bestTrajectory(:,2), 'g-', 'LineWidth', 3);  % Green path for the best trajectory
xlabel('X Position');
ylabel('Y Position');
title('Most Optimized Trajectory');
grid on;
axis equal;
hold off;

disp(['Best cost: ', num2str(bestCost)]);
disp('Size of the best optimized trajectory:');
disp(size(bestTrajectory));
